...
def get_binance_klines(symbol, interval, limit=1000):
    ...
    try:
        ...
        params = {
            'symbol': symbol,
            'interval': interval,
            'limit': min(limit, 1000)  # Binance API 限制最多1000條
        }
...
def get_binance_symbols():
    ...
    try:
        ...
        for symbol_info in data['symbols']:
            if ('USDT' in symbol_info['symbol'] and 
...
with col1:
    st.markdown("**波動率統計**")
    st.markdown(f"- 當日24h波動率: {today_vol:.3%}")
    st.markdown(f"- 歷史平均{period_name}波動率: {mean_vol:.3%}")
    st.markdown(f"- 波動率標準差: {std_vol:.3%}")
    st.markdown(f"- 極端波動門檻 (±2σ): {2 * std_vol:.3%}")
    st.markdown(f"- 當日波動率歷史百分位: {today_percentile:.1f}%")
